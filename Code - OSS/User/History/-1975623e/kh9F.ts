...
export class Pawn extends Figure {
  ...
  canMove(target: Cell): boolean {
    ...
    if (target.y === this.cell.y + direction) {
      ...
      if (this.isFirstStep && target.x === this.cell.x && this.cell.board.getCell(target.x, target.y).isEmpty() 
          && this.cell.board.getCell(target.x, this.cell.y + firstStepDirection).isEmpty()) {
        return true;
      }
